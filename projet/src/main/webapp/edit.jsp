...
if (request.getParameter("submitBtn") != null) {
    ...
    try {
        ...
        PreparedStatement pst = conx.prepareStatement("UPDATE affection SET id_post=?, shift=?, date_affectation=? WHERE id_navire=?");
        pst.setString(1, idPost);
        pst.setString(2, shift);
        pst.setString(3, dateAffectation);
        pst.setString(4, idNavire);
        pst.executeUpdate();
...
<html>
  ...
  <%  
            ...
            if (idNavire != null) {
                Connection conx = Utilconx.Seconnecter();
                PreparedStatement pst = conx.prepareStatement("SELECT id_navire, id_post, shift, date_affectation FROM affection WHERE id_navire = ?");
                ...
                if (rs.next()) {
                    ...
                    String dateAffectation = rs.getString("date_affectation");
        %>
    ...
    <% } %>
                ...
                <input type="text" name="shift" placeholder="shift" value="<%= shift %>" class="form-control" required>
               
                <input type="text" name="date" value="<%= dateAffectation %>"placeholder="date_affectation" class="form-control" required>
