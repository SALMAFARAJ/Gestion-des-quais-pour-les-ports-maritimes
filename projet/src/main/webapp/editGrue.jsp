...
if (request.getParameter("submitBtn") != null) {
    ...
    try {
        ...
        String id_gr = request.getParameter("id_gr");
        
        String date_gr = request.getParameter("date_gr");
        Connection conx = Utilconx.Seconnecter();
        PreparedStatement pst = conx.prepareStatement("UPDATE affectation_gr SET id_gr=?,  date_gr=? WHERE id_navire=?");
        pst.setString(1, id_gr);
        
        pst.setString(2, date_gr);
        pst.setString(3, idNavire);
        pst.executeUpdate();
       
        conx.close();
        response.sendRedirect("Grue.jsp");
    }catch(Exception e){}}
%>
...
<html>
  ...
  <%  
            ...
            if (idNavire != null) {
                Connection conx = Utilconx.Seconnecter();
                PreparedStatement pst = conx.prepareStatement("SELECT id_navire, id_gr, date_gr FROM affectation_gr WHERE id_navire = ?");
                pst.setString(1, idNavire);
                ResultSet rs = pst.executeQuery();

                if (rs.next()) {
                	String id_gr=rs.getString("id_gr");
                    
                    String date_gr = rs.getString("date_gr");
                           %>
            ...
            <input type="hidden" name="id_navire" value="<%= idNavire %>">
              ...
              <select name="id_gr" class="form-control" id="id_grue" required>
    <% 
    // Code Java to retrieve the available posts from the database
    Connection co = Utilconx.Seconnecter();
    PreparedStatement ps = co.prepareStatement("SELECT id_gr from grue where id_gr not in(SELECT id_gr from affectation_gr )");
    ...
    while (rst.next()) {
        ...
        <option value="<%= postId %>" <%= (postId.equals(id_gr)) ? "selected" : "" %>><%= postId %></option>
        <% 
        // Set the idPostFound to true if the id_post exists in the database
        if (postId.equals(id_gr)) {
            idPostFound = true;
        }
    } %>
    <!-- If the id_post does not exist in the database, display it directly -->
    <% if (!idPostFound && id_gr != null) { %>
        <option value="<%= id_gr %>" selected><%= id_gr %></option>
    <% } %>
</select>
                ...
                <input type="text" name="date_gr" value="<%= date_gr %>"placeholder="date_affectation" class="form-control" required>
